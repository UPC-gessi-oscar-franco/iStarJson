...
public class ccistarmlFile {
    ...
    public boolean remove_diagramByID(String id){
        ...
        if (this.xml.content.size()<2) return false;
        x = (ccistarmlContent)this.xml.content.get(1);
        if (!x.tagName.equals("istarml")) return false;
    ...
    public boolean saveFile(String fileName){
        try {
            ...
            return true;
             ...
             this.error.addLast(new ccfileError("File input error on "+fileName));
             return false;
        }
    }
    ...
    public boolean loadFile(String fileName){
        try {
            ...
            rd = f.read();
            while (rd!=-1){
                if (this.text.capacity()>0) this.text.append((char)rd);
                else er++;
                rd = f.read();
            }
            f.close();
            ...
            if (er>0){
                this.error.addLast(new ccfileError("fileName "+fileName+" too long"));
                return false;
            }
            this.length = this.text.length();
            this.position = 0;
    ...
    public String buildXML() {
        String ret="";
        if (this.xml!=null) ret=this.xml.buildXML();
        return ret;
    }
    ...
    public ccistarmlContent mainTagStructure(){
        ...
        if (this.xml.content == null) return null;
        if (this.xml.content.size()<=1) return null;
